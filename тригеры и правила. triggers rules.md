...
Пояснения:
* `for each row` в тригере означает, что будут затронуты тригером ВСЕ строки, переданные на обновление или удаление
* `new` и `old` — новые и старые данные текущей строки
...
on client
for each row -- это означает, что будут затронуты тригеры ВСЕ строки, переданные на обновление или удаление
