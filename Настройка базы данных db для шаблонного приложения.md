...
Для настройки базы данных, нам обязательно нужно определить конфиг проекта ([[Шаблонная конфигурация проекта FastAPI (core.config)]]), для которого же нужно определить `env` файл ([[Шаблонные env файлы]])

После того, как мы их определили мы должны создать файл `core/db/dependencies/db_helper.py`, который будет создавать сессии для подключения к базе данных.

```python

# ruff: noqa: I001
# ^^^^^^^^^^^^^^^^
# off authosorting imports in this file 
...
2. Проверяем в тестовом модуле работу получения `session` через механизм `Dependencies`:
```python
