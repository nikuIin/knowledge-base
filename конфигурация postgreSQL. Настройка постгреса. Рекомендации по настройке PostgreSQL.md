...
```bash
cd /etc/postgresql/17/main/postgresql.conf
```
...
### Очень важно еще поставить ограничение на длительность транзакции (чтобы случайно открытые не весели постоянно и не блокировали ничего)

Настраивается в конфигурации: https://www.postgresql.org/docs/current/runtime-config-client.html#GUC-IDLE-IN-TRANSACTION-SESSION-TIMEOUT