...
1. Создаём `env` файлы:
	* в корневой папке проекта (пропустим, так как пишем шаблон только для бэкэнда)
	* в каждом сервисе (в нашем случае в `backend`)
2. Определим `/src/core/config.py`, где будут прописаны все настройки нашего проекта:
